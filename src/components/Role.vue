<template>
  <div class>
    <div class="w-100">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h4 class="pt-2 pl-2" style="    color: #232425 !important;">Roles</h4>
        </div>
    ...
    <div class="overflow-auto">
        ...
        <template v-slot:cell(actions)="row">
          <div v-if="row.field.isAdmin" class="text_center">
            ...
            <b-dropdown id="dropdown-1" text variant="white" class="p-0">
              <b-dropdown-item @click="handleActive(row.item)" v-b-modal.roleCoinsModal>Coins</b-dropdown-item>
               <b-dropdown-item @click="handleActive(row.item)" v-b-modal.roleCoinsModal>Post Coins</b-dropdown-item>
                <b-dropdown-item @click="handleActive(row.item)" v-b-modal.roleCoinsModal>Withdraw Coins</b-dropdown-item>
            </b-dropdown>-->
          </div>
      ...
      <h5 class="text-center w-100 bg-light px-3 py-3"  v-if="!roleData && !isBusy">{{tableMsg}}</h5>
    </div>

    <!-- <roleModal :role="role" :isEdit="isEdit" @close="close" ></roleModal> -->

    <!-- <InviteeModal :roleId="role"></InviteeModal> -->
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.getRoles("roles?language=عربى");
  ...
  watch: {
    ...
    //   let link = "roles/all?limit=10&page="+this.currentPage+"&search="+this.search
    //   this.getRoles(link);
    // }
  },
  methods: {
    ...
    getRoles(link) {
      this.isBusy = true;
      this.$store
        .dispatch("getRoles",link)
        .then(data => {
          console.log("data->",data)  // eslint-disable-line no-console
          this.tableMsg=data.message
          this.roleData = data.data;
          this.filteredData = this.roleData;
